...
public class FlinkExample {
    ...
    public static void main(String[] args) throws Exception {
        ...
        KafkaSource<Tuple2<Id, User>> kafkaSource = FlinkFactory.buildKafkaSource(INPUT_TOPIC, Id.class, User.class);
        KafkaSink<Tuple2<Id, Client>> kafkaSink = FlinkFactory.buildKafkaSink(OUTPUT_TOPIC, Id.class, Client.class);
        TypeHint<Tuple2<Id, Client>> typeHint = new TypeHint<>() {
        };
    ...
    private static Tuple2<Id, Client> convertToClient(Tuple2<Id, User> idUserTuple2) {
        User user = idUserTuple2.f1;
        Client client = new Client(UUID.randomUUID().toString(), user.getId(), user.getFirstname(), user.getLastname(), user.getPhoneNumber());
        System.out.println(String.format("User: %s, Client: %s", user, client));
